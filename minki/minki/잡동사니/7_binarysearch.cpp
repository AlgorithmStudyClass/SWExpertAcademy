...
int recurbinarySearch(int *array, int target, int min, int max)
{
	...
	if (array[midpoint] > target)
	{
		return recurbinarySearch(array, target, min, midpoint - 1);
	}
	else if (array[midpoint] < target)
	{
		return recurbinarySearch(array, target, midpoint + 1, max);
	}
	else
		return midpoint;
}
int main()
{
	...
	for (int count = 0; count < numTestValues; ++count)
	{
		...
		int index = recurbinarySearch(array, testValues[count], 0, 14);
